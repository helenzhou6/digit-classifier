...
ssh -i <name of pem file>.pem.pem ec2-user@<public IP address>
sudo yum update
...
- Obviously more reading up on machine learning algorithms and make the model more effective
...
- Explore ways to get the Docker image size even smaller (especially the modelapp one, that is around 5GB currently. Managed to get it down from around 11GB but the smaller the better!)
